export const UI = {
    ...
    renderTree: (items, container, currentId, onSelect, onMove, dirtyIds, editMode, expandedIds, toggleExpand) => {
        ...
        sortedItems.forEach(item => {
            ...
            if (item.type === 'priceplan') icon = '📊'; // NYTT IKON
            ...
            div.onclick = (e) => {
                e.stopPropagation();
                if (['year','season','segment'].includes(item.type)) { toggleExpand(item.id); } else { onSelect(item); }
            };
            if (editMode) {
                div.draggable = true;
                div.ondragstart = (e) => { e.dataTransfer.setData('text/plain', item.id); e.dataTransfer.effectAllowed = 'move'; div.classList.add('dragging'); };
                div.ondragend = () => div.classList.remove('dragging');
                div.ondragover = (e) => { e.preventDefault(); e.stopPropagation(); div.classList.add('drag-over'); };
                div.ondragleave = () => div.classList.remove('drag-over');
                div.ondrop = (e) => { e.preventDefault(); e.stopPropagation(); div.classList.remove('drag-over'); onMove(e.dataTransfer.getData('text/plain'), item.id, item.type); };
            }
            li.appendChild(div);
    ...
    renderCharts: (containerId, data, maxVal) => {
        ...
        container.innerHTML = '';
        data.forEach(item => {
            const wrapper = document.createElement('div'); wrapper.className = 'chart-bar-wrapper';
            const bar = document.createElement('div'); bar.className = 'chart-bar';
            const low = ['Jan', 'Feb', 'Nov', 'Des']; const mid = ['Mar', 'Apr', 'Mai', 'Okt']; 
            if (low.includes(item.label)) bar.style.background = "#3182CE"; else if (mid.includes(item.label)) bar.style.background = "#DD6B20"; else bar.style.background = "#38A169"; 
            bar.style.height = (item.value / maxVal * 100) + '%'; bar.setAttribute('data-value', item.display || item.value);
            const label = document.createElement('div'); label.className = 'chart-label'; label.innerText = item.label;
